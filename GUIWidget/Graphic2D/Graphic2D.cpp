...
int Graphic2D::G2DShowBmp( char* disBuffer, int dw, int dh, int x, int y, int w, int h, char *bmpBuff, int bw, int bh)
{
    int drawW = 0;
    int drawH = 0;
    int tempPoint;
    ...
    for( int h = 0; h < drawH; h++ )
    {
        for( int w = 0; w < drawW; w++ )
        {
            tempPoint = ((int)(h*scaleH))*bw*3+((int)(w*scaleW))*3;
            disBuffer[dw*4*(h+y)+(w+x)*4]   = bmpBuff[tempPoint+0];
            disBuffer[dw*4*(h+y)+(w+x)*4+1] = bmpBuff[tempPoint+1];
            disBuffer[dw*4*(h+y)+(w+x)*4+2] = bmpBuff[tempPoint+2];
            disBuffer[dw*4*(h+y)+(w+x)*4+3] = 255;
        }
    } 
}
